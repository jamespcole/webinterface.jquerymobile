...
(function($) {  
  ...
  $.extend(xbmc, {
    ...
    getEpisodes: function(options) {
     ...
     // settings.order = awxUI.settings.epdesc;
      ...
      $.extend(settings, options);
      console.log('{"jsonrpc": "2.0", "method": "VideoLibrary.GetEpisodes", "params": { ' + (settings.item == ''? (settings.filter != ''? settings.filter : '') : (settings.filter != ''? settings.filter + ', ' : '') + '"' + settings.item + '": ' + (settings.itemId != -1? (settings.season != -1? settings.itemId + ', "season": ' + settings.season : settings.itemId) : (settings.season != -1? '"season": ' + settings.season + ', ' + '"' + settings.itemStr + '"' : '"' + settings.itemStr + '"') ) ) + ', "limits": { "start" : ' + settings.start + ', "end": ' + settings.end + ' }, "properties": ["episode", "playcount", "fanart", "plot", "season", "showtitle", "thumbnail", "rating"], "sort": { "order": "' + settings.order + '", "method": "' + settings.sortby + '" } }, "id": "libEps"}');
      xbmc.sendCommand(
  ...
  $.extend(xbmc, {
    pollTime: function() {
      ...
      if ((xbmc.periodicUpdater.loopCount % 15) == 0 && xbmc.periodicUpdater.loopCount > 14 && xbmc.xart.length > 0) {
        xbmc.switchFanart();
      }
  ...
  $.extend(xbmc, {    
    ...
    wsListener: function() {
        ...
        var useFanart = true;//mkf.cookieSettings.get('usefanart', 'no')=='yes'? true : false;
        var showInfoTags = true;//awxUI.settings.showTags;
                ...
                function (response) {
                  ...
                  if (xbmc.periodicUpdater.currentlyPlayingFile != currentItem.file) {
                    ...
                    if (xbmc.activePlayer == 'video' && showInfoTags) {
                      ...
                      if (currentItem.streamdetails != undefined && currentItem.streamdetails.video.length != 0) {
                        if (currentItem.streamdetails != null) {
