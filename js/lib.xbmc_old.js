...
(function($) {
	...
	$.extend(xbmc, {
		...
		control: function(options) {
			...
			var commands = {play: 'PlayPause', stop: 'Stop', prev: 'GoPrevious', next: 'GoNext', shuffle: 'Shuffle', unshuffle: 'Unshuffle', left: 'MoveLeft', right: 'MoveRight', zoom : 'Zoom'};
		...
		playAudio: function(options) {
			...
			xbmc.sendCommand(
				'{"jsonrpc": "2.0", "method": "Player.Open", "params" : { "item" : { "playlistid" : 0, "position": ' + settings.item + ' } }, "id": 0}',
				settings.onSuccess,
				function(response) {
		...
		removeAudioPlaylistItem: function(options) {
			...
			xbmc.sendCommand(
				'{"jsonrpc": "2.0", "method": "Playlist.Remove", "params" : { "playlistid" : 0, "position": ' + settings.item + ' }, "id": 0}',
				settings.onSuccess,
				function(response) {
					settings.onError("failed to remove item");
				}
			);
		},
			...
			this.clearAudioPlaylist({
				onSuccess: function() {
					xbmc.addSongToPlaylist({
						...
						onError: function() {
							settings.onError('message_failed_add_song_to_playlist');
						}
					});
				},

				onError: function() {
					settings.onError('message_failed_clear_playlist');
				}
			});
		},
		...
		getAlbumDetails: function(options) {
			...
			xbmc.sendCommand(
					'{"jsonrpc": "2.0", "method": "AudioLibrary.GetAlbumDetails", "params": {"albumid": ' + settings.albumid + ', "properties": ["artist", "genre", "rating", "thumbnail", "year", "mood", "style"]}, "id": 1}',
		...
		getApiVersion:  function(options) {
			var settings = {				
				onSuccess: null,
				onError: null
			};
			...
			xbmc.sendCommand(
				'{"jsonrpc": "2.0", "method": "JSONRPC.Version",  "id": 1}',
					function(response) {
						...
						apiVersion = response.result.version.major;
					settings.onSuccess(response.result);
				},
